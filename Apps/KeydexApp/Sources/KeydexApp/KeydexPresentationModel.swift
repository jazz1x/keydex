  ...
  ShellSettingsConfig(
    ...
    scanSources: [
      ScanSourceRow(
        ...
        title: "Keychain",
        detail: "Track configured macOS Keychain references without secret values",
        enabled: true
      ),
      ScanSourceRow(
...
func localSettingsData(displayMode: InventoryDisplayMode = .cards) -> ShellSettingsConfig {
  var settings = sampleSettingsData(displayMode: displayMode)
  settings.keychainAccess = false
  settings.requestPrompt = false
  settings.keychainReferences = []
  settings.scanSources = settings.scanSources.map { source in
    var source = source
