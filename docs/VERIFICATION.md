...
| App Accessibility Evidence Status | `scripts/app-accessibility-evidence-status.sh` | Current local accessibility evidence is listed per scenario and field, with notes and review audit keys present, and the next pending scenario/field/notes/screenshot/review-command set is reported so manual review work remains visible and resumable. |
...
| Release Smoke | `scripts/release-smoke.sh` | Release-mode artifacts bundle locally, ad-hoc sign cleanly, create unsigned DMG smoke evidence, retry transient `hdiutil verify` resource-busy locks, checksum cleanly, and omit fixture metadata plus fake secret sentinels. |
