...
| App Accessibility Smoke | `scripts/app-accessibility-smoke.sh` | Running app exposes core sidebar, card library, card detail, table, doctor, inspector, settings, and state names through the macOS `AXUIElement` tree, and keeps toolbar controls hidden from AX while settings is modal. |
