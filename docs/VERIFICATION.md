...
| App Accessibility Smoke | `scripts/app-accessibility-smoke.sh` | Running app exposes the full shared scenario set through the macOS `AXUIElement` tree, including card library, card detail, empty, search, inspector, settings subsections, compact window sizing, toolbar controls hidden from AX while settings is modal, and per-scenario diagnostics for local AX crashes. |
...
| Forbidden Patterns | `scripts/forbidden-patterns.sh` | No silent `try?`, script-level `try!`, empty `catch`, or obvious secret-value metadata. |
