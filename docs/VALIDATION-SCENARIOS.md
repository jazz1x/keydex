...
| B9 | Local release artifact builds with app bundle, DMG, and checksum evidence. | `scripts/release-smoke.sh`. |
...
`tmp/release-smoke` archive, unsigned app bundle, unsigned DMG, and SHA-256 checksums
from release-mode SwiftPM products, then inspects the archive file list before any public
release is cut.
